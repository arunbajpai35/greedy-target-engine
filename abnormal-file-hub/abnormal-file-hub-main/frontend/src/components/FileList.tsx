...
export const FileList: React.FC = () => {
  ...
  const { data: files, isLoading, error } = useQuery({
    queryKey: ['files', { search, fileType, sizeMin, sizeMax, dateFrom, dateTo }],
    queryFn: () =>
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-4 rounded-md shadow mb-4 grid grid-cols-1 md:grid-cols-6 gap-3">
        ...
        <div className="md:col-span-6 flex gap-2">
          ...
          <button onClick={handleReset} className="px-3 py-1 bg-gray-200 rounded">
            Reset
          </button>
        </div>
      </div>
              ...
              <li key={file.id} className="py-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {'is_duplicate' in file && (file as any).is_duplicate && (
                      <p className="text-xs text-yellow-700">Duplicate of {(file as any).duplicate_of}</p>
                    )}
                  </div>
